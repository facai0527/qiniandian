// User-supplied React Bits AeroShards; QND also supports a crisp native-vector compositor path.
import React, { useEffect, useRef, useState } from 'react';
import { draw, effect, frame, init, sampler, surface, target, uniforms } from 'vgpu';

import './AeroShards.css';
import { startAeroCompositor } from './aero-compositor';
...
const QUALITY_PRESETS = {
  low: { count: 320, dpr: 1.5, supersamplePixels: 1400000 },
  medium: { count: 480, dpr: 1.5, supersamplePixels: 3500000 },
  high: { count: 600, dpr: 2, supersamplePixels: 4500000 }
};
...
const FRAME_STATES = {
  ...
  partial: { interval: 1000 / 60, continuous: true }
};
...
fn vs_main(
  ...
  if (corner == 0u) {
    ...
    if (materialKind > 0.5 && materialKind < 1.5) {
      ...
      // Preserve the chosen facet palette rather than washing all shards into grey.
      let seedTint = unitFloat(instanceIndex * 1013904223u + 2246822519u);
      let paletteColor = mix(view.baseColor.rgb, view.accentColor.rgb, step(0.70, seedTint));
      let satinColor = paletteColor * (0.64 + diffuse * 0.36);
      color = mix(color, satinColor, 0.82);
      creaseColor = mix(creaseColor, satinColor, 0.82);
    }
...
const resolveDpr = (preset, canvas) => {
  const cssPixels = Math.max(1, canvas.clientWidth * canvas.clientHeight);
  // Preserve at least CSS-native detail. Reduce particle work, never blur the entire layer.
...
const prepareRenderGraph = async (graph, outputFormat, settings) => {
  if(settings.effect===EFFECTS.none && !settings.bloom && !settings.grain && !settings.chromaticAberration){
    await graph.shardDraw.compile({ colors: [outputFormat] });
    return;
  }
  await Promise.all([
...
export default function AeroShards({
  ...
  useEffect(() => {
    ...
    // QND uses native-vector stream animation, independent of the GLB's render loop.
    if(compositor){
      try{
        const engine=startAeroCompositor({canvas,root,settingsRef,onReady:()=>setReady(true)});
        wakeRef.current=engine.update;
        return()=>{wakeRef.current=()=>{};engine.dispose();};
      }catch(error){onErrorRef.current?.(error);return;}
    }
    ...
    void (async () => {
      try {
        ...
        const output = surface(gpu, canvas, {
          ...
          autoResize: false,
          // The component always clears to an opaque background; avoid unnecessary alpha compositing.
          alphaMode: 'opaque',
          format: outputFormat
        });
        const graph = createRenderGraph(gpu, output.size, resolveBloomSize([canvas.clientWidth, canvas.clientHeight]));
        await prepareRenderGraph(graph, outputFormat, settingsRef.current);
        if (disposed) return;
        ...
        const renderFrame = currentFrame => {
          ...
          canvas.__aeroCount = activeCount;
        ...
        const scheduleFrame = timestamp => {
          ...
          try {
            ...
            canvas.__aeroFrames = (canvas.__aeroFrames || 0) + 1;
            canvas.__aeroDistance = flowDistance;
            canvas.__aeroTimestamp = timestamp;
          } catch (error) {
            reportFailure(error);
            return;
          }
